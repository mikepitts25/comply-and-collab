...
- **Policy Health** — every 800-53 family requires a documented, periodically reviewed policy (AC-1, AU-1, …). Map POLICY documents to those controls with a review cadence (6/12/24 months); approval starts the re-review clock, and a 20-family traffic-light board shows Current / Review overdue / In progress / Missing per system, with overdue reviews on the dashboard.
- **Compliance Calendar** — every dated obligation in one feed: ATO expirations, POA&M completions, milestones, ISA expirations, risk-acceptance reviews, and policy/document re-reviews — grouped by month with overdue flags, and exportable as **.ics for Outlook**.
- **Document library with review workflow** — versioned ATO-package documents (policies, plans, diagrams, letters) with per-version change tracking: every upload records **who, when, and a required "what changed" note**, SHA-256 hashed, any version downloadable. Review flow **Draft → Ready for review → In review → Approved / Changes requested (→ Archived)** with senior-role gating on review decisions, a per-document event timeline with comments, and automatic approval reset when content changes.
